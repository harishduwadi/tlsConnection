...
func certbotController(db *db.Dbmanager, certConfig *certbotconfig.Configuration) {
	...
	for _, cert := range requestCertConfig.Certificates {
		...
		rawcert, err := certpackage.CreateValidCertificate(acmeClient)
		if err != nil {
			log.Fatal(err)
		}
		...
		err = updateCertificate(db, rawcert)
		if err != nil {
			log.Fatal(err)
		}
...
func populateDB(db *db.Dbmanager, certConfig *certbotconfig.Configuration) {
	...
	for _, xmlCert := range certConfig.Certificates {
		...
		if len(ips) == 0 {
			log.Fatalln(xmlCert.CommonName, "doesn't have any ip address")
		}
		...
		if err != nil {
			log.Printf("Connection error with %s: %v\n", xmlCert.CommonName, err)
			continue
		}
		...
		log.Println("Adding Entry to DB wih commonname:", certificate.Subject, "and config name", xmlCert.CommonName)

		// Add/Retrieve Key Table entry
		keyid, err := updateKeyTable(db, xmlCert.Credential)
		if err != nil {
			log.Println(err)
			return
		}
		...
		certificateID, err := updateCertificateTable(db, certificate, keyid)
		if err != nil {
			log.Println(err)
			return
		}
...
func updateKeyTable(db *db.Dbmanager, credential certbotconfig.CredentialConfiguration) (int, error) {
	id, err := db.GetKeyId(credential.APIKey, credential.APISecret)
	if err != nil {
		return id, err
	}
	if id == -1 {
		err = db.AddKeyEntry(credential.APIKey, credential.APIKeyEnvironmentVariableName, credential.APISecret, credential.APISecretEnvironmentVariableName, credential.DNSProviderName)
		if err != nil {
			return -1, err
		}
		id, err = db.GetKeyId(credential.APIKey, credential.APISecret)
...
func updateCertificate(db *db.Dbmanager, rawCert []byte) error {
	...
	err = db.UpdateCertificateEntry(certificate.Subject.CommonName, serialNumberInHex, certificate.NotBefore, rawCert, certificate.Issuer.CommonName)
	if err != nil {
		return err
	}
...
func updateCertificateTable(db *db.Dbmanager, certificate *config.CertificateEntry, keyid int) (int, error) {
	certificateID, err := db.GetCertificateEntry(certificate.Subject)
	if err != nil {
		return -1, err
	}
	if certificateID == -1 {
		err = db.AddCertificateEntry(certificate.Subject, certificate.SerialNumber, certificate.Validity, nil, certificate.Issuer, keyid)
		if err != nil {
			return -1, err
		}
		certificateID, err = db.GetCertificateEntry(certificate.Subject)
